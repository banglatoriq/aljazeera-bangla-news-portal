...
@st.cache_resource
def init_db():
    conn = sqlite3.connect('news_db_v6_free.db', check_same_thread=False)
    c = conn.cursor()
    # video_url এবং embed_code কলাম যুক্ত করা হয়েছে
    c.execute('''CREATE TABLE IF NOT EXISTS news_table
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, link TEXT, translated_title TEXT, 
                  full_text TEXT, image_url TEXT, video_url TEXT, embed_code TEXT, source TEXT, date TIMESTAMP)''')
...
# --- ফ্রি অনুবাদ ফাংশন (Sentence-based) ---
def safe_translate(text):
    ...
    try:
        ...
        if len(text) > 1500:
            ...
            translated_chunks = [translator.translate(s) for s in sentences if s.strip()]
...
# --- উন্নত স্ক্র্যাপিং (টুইট ও ভিডিও সাপোর্টসহ) ---
def scrape_news():
    news_feeds = {
            ...
            for entry in feed.entries[:5]:
                ...
                if not c.fetchone():
                    try:
                        ...
                        # টুইট বা ইউটিউব ভিডিও লিঙ্ক খোঁজা
                        embed_link = None
                        video_tag = art_soup.find('iframe')
                        if video_tag and video_tag.get('src'):
                            embed_link = video_tag.get('src')
                        ...
                        c.execute('''INSERT INTO news_table (title, link, translated_title, full_text, image_url, video_url, source, date) 
                                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)''', 
                                  (entry.title, entry.link, bn_title, bn_full_text, img, embed_link, source_name, datetime.now()))
                        conn.commit()
                    except: continue
        except: continue
...
if st.session_state.view == 'home':
    show_logo()
    c.execute("SELECT id, translated_title, image_url, source, date FROM news_table ORDER BY date DESC LIMIT 12")
    all_news = c.fetchall()
...
# --- ২. বিস্তারিত পেইজ ---
    ...
    # 🔴 টুইট বা ভিডিও এমবেড প্রদর্শন
    if news[6]:
        st.write("---")
        st.subheader("📺 ভিডিও রিপোর্ট / সোশ্যাল এমবেড")
        # যদি এটি ইউটিউব বা সাধারণ ভিডিও হয়
        if "youtube" in news[6] or "vimeo" in news[6]:
            st.video(news[6])
        else:
            # টুইট বা অন্য লিঙ্ক হলে আইফ্রেম দিয়ে দেখানোর চেষ্টা
